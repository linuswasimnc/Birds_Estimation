...
model_load =tf.keras.models.load_model(r'model.h5',compile=False)
# Flask utils
...
def get_conservation(name):
    url = 'https://google.com/search?q='+name+"+conservation+status&hl=en"
    print(url)
...
def get_image(name):
    url = 'https://google.com/search?q='+name+"+conservation+status&tbm=isch"
    print(url)
    request_result=requests.get( url )
